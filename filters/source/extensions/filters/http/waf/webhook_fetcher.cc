...
WebhookFetcher::WebhookFetcher(Upstream::ClusterManager& cm,
                               ...
                               const std::string& secret, WebhookFetcherCallback& callback)
...
void WebhookFetcher::invoke(const std::string& body) {
  ...
  if (secret_.size()) {
    ...
    message->headers().addCopy(WebhookHeaders::get().SignatureType,
                               WebhookConstants::get().Sha256Hmac);
    message->headers().addCopy(WebhookHeaders::get().SignatureValue,
                               Hex::encode(crypto_util.getSha256Hmac(secret_, body)));
  }
...
void WebhookFetcher::onSuccess(const Http::AsyncClient::Request&,
  ...
  if (status_code == enumToInt(Http::Code::OK)) {
    ...
    ENVOY_LOG(debug, "Webhook [uri = {}]: bad response status code {}", uri_.uri(), status_code);
