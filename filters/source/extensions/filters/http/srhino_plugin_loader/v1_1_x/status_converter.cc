...
StatusConverter::convert(SrhinoPluginFramework::v1_1_x::HeaderStatus status) {
  using SrhinoStatus = SrhinoPluginFramework::v1_1_x::HeaderStatus;
...
Http::FilterDataStatus StatusConverter::convert(SrhinoPluginFramework::v1_1_x::DataStatus status) {
  using SrhinoStatus = SrhinoPluginFramework::v1_1_x::DataStatus;
...
StatusConverter::convert(SrhinoPluginFramework::v1_1_x::TrailerStatus status) {
  using SrhinoStatus = SrhinoPluginFramework::v1_1_x::TrailerStatus;
...
void StatusConverter::adjustDataStatus(FilterStatus& filter_status,
                                       SrhinoPluginFramework::v1_1_x::DataStatus& status,
                                       bool end_stream) {
  // 如果之前返回过Continue，那么之后将不再被允许继续返回WaitForBody，如果继续返回WaitForBody，
  // 那此时的行为将跟返回Continue一样。
  if (status == SrhinoPluginFramework::v1_1_x::DataStatus::Continue) {
    filter_status.is_cotinued_ = true;
  } else if (status == SrhinoPluginFramework::v1_1_x::DataStatus::WaitForBody ||
             status == SrhinoPluginFramework::v1_1_x::DataStatus::TryWaitForBody) {
    if (filter_status.is_waited_) {
      status = SrhinoPluginFramework::v1_1_x::DataStatus::Continue;
    } else {
      if (end_stream) {
        status = SrhinoPluginFramework::v1_1_x::DataStatus::Continue;
      } else {
