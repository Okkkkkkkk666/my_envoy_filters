...
void HttpGrpcAccessLog::emitLog(const Http::RequestHeaderMap& request_headers,
  ...
  const auto filter_log_iter = filter_metadata.find(FilterAccessLog::metadata_filter_name_);
  if (filter_log_iter != filter_metadata.end()) {
    auto& log_metadata = *(log_entry.mutable_common_properties()->mutable_metadata()->mutable_filter_metadata());
    log_metadata[FilterAccessLog::metadata_filter_name_].MergeFrom(filter_log_iter->second);
  }
