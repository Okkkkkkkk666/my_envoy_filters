...
HttpConnectionManagerConfig::HttpConnectionManagerConfig(
    ...
    : context_(context), stats_prefix_(fmt::format("http.{}.", config.stat_prefix())),
      ...
      xff_num_trusted_hops_(config.xff_num_trusted_hops() % XffShiftPosition),
      xff_trusted_hops_shift_(
          (config.xff_num_trusted_hops() % (XffShiftPosition * 10)) / XffShiftPosition == 1),
      xff_replace_(
          (config.xff_num_trusted_hops() % (XffReplacePosition * 10)) / XffReplacePosition == 1),
      skip_xff_append_(config.skip_xff_append()),
      use_x_real_ip_(
          (config.xff_num_trusted_hops() % (XffXRealIpPosition * 10)) / XffXRealIpPosition == 1),
      via_(config.via()), route_config_provider_manager_(route_config_provider_manager),
  ...
  if (config.has_tracing()) {
    ...
    std::set<std::string> span_tags;
    for (int i = 0; i < tracing_config.span_tags_size(); i++) {
      span_tags.insert(tracing_config.span_tags(i));
    }
    ...
    tracing_config_ =
        std::make_unique<Http::TracingConnectionManagerConfig>(Http::TracingConnectionManagerConfig{
            ...
            tracing_config.verbose(), max_path_tag_length, span_tags});
  }
