...
public:
  ...
  bool xffReplace() const override { return xff_replace_; }
  bool xffTrustedHopsShift() const override { return xff_trusted_hops_shift_; }
  bool skipXffAppend() const override { return skip_xff_append_; }
  bool useXRealIp() const override { return use_x_real_ip_; }
...
private:
  ...
  // 改变取xff方向，true时，从左边开始取
  const bool xff_trusted_hops_shift_;
  // 替换xff
  const bool xff_replace_;
  const bool skip_xff_append_;
  const bool use_x_real_ip_;
  ...
  static const uint32_t XffReplacePosition = 100000;
  static const uint32_t XffShiftPosition = 10000;
  static const uint32_t XffXRealIpPosition = 1000000;
};
