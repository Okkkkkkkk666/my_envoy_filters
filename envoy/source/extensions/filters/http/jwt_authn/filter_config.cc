...
FilterConfigImpl::FilterConfigImpl(
  ...
  for (const auto& rule : proto_config_.rules()) {
    ...
    case RequirementRule::RequirementTypeCase::kRequirement:
      rule_pairs_.emplace_back(Matcher::create(rule),
                               Verifier::create(rule.requirement(), proto_config_.providers(), *this));
      break;
  ...
  if (proto_config_.has_filter_state_rules()) {
    ...
    for (const auto& it : proto_config_.filter_state_rules().requirements()) {
