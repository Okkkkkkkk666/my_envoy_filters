...
ContextConfigImpl::ContextConfigImpl(
  ...
  std::unordered_set<std::string> gm_cipher_suites{"RSA_SM4_CBC_SHA256",  "RSA_SM4_CBC_SM3",
                                            "ECC_SM2_SM4_CBC_SM3", "ECDHE_SM2_SM4_CBC_SM3",
                                            "ECC_SM2_SM4_GCM_SM3", "ECDHE_SM2_SM4_GCM_SM3",
                                            "RSA_SM4_GCM_SHA256",  "RSA_SM4_GCM_SM3"};
  auto cipher_suites = config.tls_params().cipher_suites();
  if (!cipher_suites.empty()) {
      if (gm_cipher_suites.find(cipher_suites[0]) != gm_cipher_suites.end()) {
        is_tlcp_ = true;
      }
  }
  cipher_suites_ = StringUtil::nonEmptyStringOrDefault(RepeatedPtrUtil::join(cipher_suites, ":"),
...
const unsigned ClientContextConfigImpl::DEFAULT_MAX_VERSION = TLS1_3_VERSION;
...
ServerContextConfigImpl::ServerContextConfigImpl(
  ...
  if (config.has_session_timeout()) {
    ...
    session_timeout_ = std::chrono::seconds(2 * 60 * 60);
  }
}
