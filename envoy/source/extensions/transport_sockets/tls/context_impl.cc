...
ContextImpl::ContextImpl(Stats::Scope& scope, const Envoy::Ssl::ContextConfig& config,
  ...
  if (!capabilities_.provides_certificates) {
    for (uint32_t i = 0; i < tls_certificates.size(); ++i) {
      ...
      case EVP_PKEY_EC: {
        ...
        if (EC_GROUP_get_curve_name(ecdsa_group) == NID_sm2) {
          // sm2曲线
          break;
        } else if (ecdsa_group == nullptr ||
          ...
          throw EnvoyException(fmt::format("Failed to load certificate chain from {}, only P-256 "
                                           "ECDSA certificates are supported, is {}",
                                           ctx.cert_chain_file_path_,
                                           EC_GROUP_get_curve_name(ecdsa_group)));
        }
  ...
  stat_name_set_->rememberBuiltins({"TLS_AES_128_GCM_SHA256", "TLS_AES_256_GCM_SHA384",
                                    "TLS_CHACHA20_POLY1305_SHA256", "TLS_SM4_GCM_SM3",
                                    "TLS_SM4_CCM_SM3"});
  ...
  stat_name_set_->rememberBuiltins({"P-224", "P-256", "P-384", "P-521", "X25519", "CECPQ2", "CurveSM2"});
...
void TlsContext::loadCertificateChain(const std::string& data, const std::string& data_path) {
  ...
  auto cert_data = Utility::CertificateParser::parse(data);
  if (!cert_data.is_gm) {
    ...
    std::string sign_cert = cert_data.sign;
    std::string enc_cert = cert_data.enc;
    // 国密tlcp证书
    bssl::UniquePtr<BIO> bio_sign(
        BIO_new_mem_buf(const_cast<char*>(sign_cert.data()), sign_cert.size()));
    bssl::UniquePtr<BIO> bio_enc(
        BIO_new_mem_buf(const_cast<char*>(enc_cert.data()), enc_cert.size()));
    RELEASE_ASSERT(bio_sign != nullptr, "");
    RELEASE_ASSERT(bio_enc != nullptr, "");
    cert_chain_.reset(PEM_read_bio_X509_AUX(bio_sign.get(), nullptr, nullptr, nullptr));
    bssl::UniquePtr<X509> enc_cert_x509(
        PEM_read_bio_X509_AUX(bio_enc.get(), nullptr, nullptr, nullptr));
    if (cert_chain_ == nullptr || enc_cert_x509 == nullptr ||
        !SSL_CTX_use_sign_certificate(ssl_ctx_.get(), cert_chain_.get()) ||
        !SSL_CTX_use_enc_certificate(ssl_ctx_.get(), enc_cert_x509.get())) {
      logSslErrorChain();
      throw EnvoyException(
          absl::StrCat("Failed to load gm certificate chain from ", cert_chain_file_path_));
    }

    // Read rest of the certificate chain.
    while (true) {
      bssl::UniquePtr<X509> cert(PEM_read_bio_X509(bio_sign.get(), nullptr, nullptr, nullptr));
      if (cert == nullptr) {
        break;
      }
      if (!SSL_CTX_add_extra_chain_cert(ssl_ctx_.get(), cert.get())) {
    ...
    while (true) {
      bssl::UniquePtr<X509> cert(PEM_read_bio_X509(bio_enc.get(), nullptr, nullptr, nullptr));
      if (cert == nullptr) {
        break;
      }
      if (!SSL_CTX_add_extra_chain_cert(ssl_ctx_.get(), cert.get())) {
...
void TlsContext::loadPrivateKey(const std::string& data, const std::string& data_path,
  ...
  if (!cert_data.is_gm) {
    ...
    std::string sign_key = cert_data.sign;
    std::string enc_key = cert_data.enc;
    bssl::UniquePtr<BIO> bio_sign(
        BIO_new_mem_buf(const_cast<char*>(sign_key.data()), sign_key.size()));
    bssl::UniquePtr<BIO> bio_enc(
        BIO_new_mem_buf(const_cast<char*>(enc_key.data()), enc_key.size()));
    auto password_data = Utility::CertificateParser::parse(password);
    RELEASE_ASSERT(bio_sign != nullptr, "");
    RELEASE_ASSERT(bio_enc != nullptr, "");
    bssl::UniquePtr<EVP_PKEY> pkey_sign(PEM_read_bio_PrivateKey(
        bio_sign.get(), nullptr, nullptr,
        !password_data.sign.empty() ? const_cast<char*>(password_data.sign.c_str()) : nullptr));
    bssl::UniquePtr<EVP_PKEY> pkey_enc(PEM_read_bio_PrivateKey(
        bio_enc.get(), nullptr, nullptr,
        !password_data.enc.empty() ? const_cast<char*>(password_data.enc.c_str()) : nullptr));

    if (pkey_sign == nullptr || pkey_enc == nullptr ||
        !SSL_CTX_use_sign_PrivateKey(ssl_ctx_.get(), pkey_sign.get()) ||
        !SSL_CTX_use_enc_PrivateKey(ssl_ctx_.get(), pkey_enc.get())) {
      throw EnvoyException(fmt::format("Failed to load private key from {}, Cause: {}", data_path,
                                       Utility::getLastCryptoError().value_or("unknown")));
    }

    checkPrivateKey(pkey_sign, data_path);
    checkPrivateKey(pkey_enc, data_path);
  }
}
