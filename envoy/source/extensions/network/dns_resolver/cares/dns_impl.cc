...
// 对envoy dns查询请求设置8577标记
static int setSoMarkCallback(ares_socket_t socket_fd, [[maybe_unused]] int type,
                             [[maybe_unused]] void* data) {
  int val = 8577;
  auto rc = setsockopt(socket_fd, SOL_SOCKET, SO_MARK, &val, sizeof(val));
  if (rc != 0) {
    std::cerr << "cares dns resolution setsockopt error: " << strerror(errno) << std::endl;
  }
  return rc;
}
...
void DnsResolverImpl::initializeChannel(ares_options* options, int optmask) {
  ...
  ares_set_socket_callback(channel_, setSoMarkCallback, nullptr);
}
