...
ListenerFactoryContextBaseImpl::ListenerFactoryContextBaseImpl(
    ...
    : server_(server), metadata_(config.metadata()), typed_metadata_(config.metadata()),
      name_(config.name()), direction_(config.traffic_direction()),
...
const std::string& ListenerFactoryContextBaseImpl::listenerName() const { return name_; }
...
ListenerImpl::ListenerImpl(const envoy::config::listener::v3::Listener& config,
    ...
    : parent_(parent), address_(Network::Address::resolveProtoAddress(config.address())),
      ...
      listener_tag_(parent_.factory_.nextListenerTag()), name_(name), stat_prefix_(config.stat_prefix()), added_via_api_(added_via_api),
...
const std::string& PerListenerFactoryContextImpl::listenerName() const {
  return listener_factory_context_base_->listenerName();
}
