...
#include "source/server/ulid_uint128.hh"
...
InstanceImpl::InstanceImpl(
    ...
    : init_manager_(init_manager), workers_started_(false), live_(false), shutdown_(false),
      ...
      original_start_time_(start_time_), ulid_(ulid::Marshal(ulid::CreateNowRand())),
      steady_start_time_(
          duration_cast<milliseconds>(steady_clock::now().time_since_epoch()).count()),
      stats_store_(store), thread_local_(tls), random_generator_(std::move(random_generator)),
      api_(new Api::Impl(
...
void InstanceImpl::updateServerStats() {
  ...
  uint64_t current_time = duration_cast<milliseconds>(steady_clock::now().time_since_epoch()).count();
  server_stats_->elapsed_time_.set(current_time - steady_start_time_);
...
void InstanceImpl::initialize(Network::Address::InstanceConstSharedPtr local_address,
  ...
  server_stats_ = std::make_unique<ServerStats>(
      ServerStats{ALL_SERVER_STATS(POOL_COUNTER_PREFIX(stats_store_, server_stats_prefix),
                                   ...
                                   POOL_TEXT_READOUT_PREFIX(stats_store_, server_stats_prefix))});
...
Runtime::LoaderPtr InstanceUtil::createRuntime(Instance& server,
  ...
  return std::make_unique<Runtime::LoaderImpl>(
      ...
      server.messageValidationContext().dynamicValidationVisitor(), server.api(), server.admin());
}
