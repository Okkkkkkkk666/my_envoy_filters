...
public:
  ...
  void logTrace() {
    ...
    std::string cmd = std::format("cat /proc/{}/maps >> /var/log/envoy_maps.txt", ::getpid());
    int ret = system("date >> /var/log/envoy_maps.txt");
    ret = system(cmd.c_str());
    ENVOY_LOG(critical, "Envoy maps stored: {}", ret == 0 ? "success" : "failed");
    ...
    if (strings == nullptr) {
      call_stack = fmt::format("backtrace_symbols fail: {}", ::strerror(errno));
    } else {
      call_stack = fmt::format("Stack trace ({} frames):\n", nptrs);
      for (int i = 0; i < nptrs; i++) {
        call_stack += fmt::format("{}\n", strings[i]);
      }
      ::free(strings);
    }
    ENVOY_LOG(critical, "{}", call_stack);
    ENVOY_LOG(critical, "-----------------------------------");
