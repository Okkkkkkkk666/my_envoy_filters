...
AdminImpl::AdminImpl(const std::string& profile_path, Server::Instance& server,
    ...
    : server_(server),
      ...
      server_info_handler_(server), srhino_plugin_handler_(config_tracker_, server),
      restricted_mode_handler_(server),
      ...
      handlers_{
          ...
          {"/srhino_plugin", "srhino plugins admin interface",
           MAKE_ADMIN_HANDLER(srhino_plugin_handler_.handlerSrhinoPluginHome), false, false},
          {"/restricted-mode/entry", "Enter restricted mode",
           MAKE_ADMIN_HANDLER(restricted_mode_handler_.entry), false, true},
          {"/restricted-mode/exit", "Exit restricted mode",
           MAKE_ADMIN_HANDLER(restricted_mode_handler_.exit), false, true},
          {"/restricted-mode/status", "Check restricted mode status",
           MAKE_ADMIN_HANDLER(restricted_mode_handler_.status), false, false},
      },
...
Http::Code AdminImpl::runCallback(absl::string_view path_and_query,
  ...
  // srhino_plugin 动态处理
  std::string_view base_path = srhino_plugin_handler_.basePath();
  if (!found_handler && strncmp(path_and_query.data(), base_path.data(), base_path.length()) == 0) {
    code = srhino_plugin_handler_.handlerSrhinoPluginUnSetHandle(path_and_query, response_headers,
                                                                 response, admin_stream);
    found_handler = true;
  }
