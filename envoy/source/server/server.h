 ...
 */
#define ALL_SERVER_STATS(COUNTER, GAUGE, HISTOGRAM, TEXT_READOUT)                                  \
  ...
  GAUGE(elapsed_time, Accumulate)                                                                  \
  TEXT_READOUT(ulid)

struct ServerStats {
  ALL_SERVER_STATS(GENERATE_COUNTER_STRUCT, GENERATE_GAUGE_STRUCT, GENERATE_HISTOGRAM_STRUCT, GENERATE_TEXT_READOUT_STRUCT)
};
...
private:
  ...
  const uint64_t steady_start_time_;
