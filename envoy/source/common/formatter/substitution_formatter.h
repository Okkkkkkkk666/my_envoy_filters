...
const std::string JsonFormatterFieldNameProtoFormat{"srhino_proto_format"};
const std::string JsonFormatterFieldNameSendToNsq{"srhino_send_to_nsq"};
#define ID_MAX_LENGTH (16)
struct PacketHeader {
  uint32_t length;
  uint8_t id[ID_MAX_LENGTH];
  uint16_t total_packets;
  uint16_t current_packet_idx;
}  __attribute__((packed));
#define PACKET_MAX_SIZE (10 * 1024 * 1024 - (sizeof(struct PacketHeader)))
...
public:
  JsonFormatterImpl(const ProtobufWkt::Struct& format_mapping, bool preserve_types,
      ...
      : struct_formatter_(format_mapping, preserve_types, omit_empty_values),
        is_proto_format_(findField(format_mapping, JsonFormatterFieldNameProtoFormat)),
        is_send_to_nsq_(findField(format_mapping, JsonFormatterFieldNameSendToNsq)) {}
  JsonFormatterImpl(const ProtobufWkt::Struct& format_mapping, bool preserve_types,
                    bool omit_empty_values, const std::vector<CommandParserPtr>& commands)
  ...
  std::string format(const Http::RequestHeaderMap& request_headers,
                     ...
                     const StreamInfo::StreamInfo& stream_info,
                     absl::string_view local_reply_body) const override;
  bool isSendToNsq() const override { return is_send_to_nsq_; }

private:
  bool findField(const ProtobufWkt::Struct& struct_format, const std::string& field_name) const;

private:
  const StructFormatter struct_formatter_;
  bool is_proto_format_{false};
  bool is_send_to_nsq_{false};
};

class ProtoFormatterImpl : public Formatter {
public:
  ProtoFormatterImpl(const ProtobufWkt::Struct& format_mapping, bool preserve_types,
                    bool omit_empty_values)
      : struct_formatter_(format_mapping, preserve_types, omit_empty_values) {}
  ProtoFormatterImpl(const ProtobufWkt::Struct& format_mapping, bool preserve_types,
                    bool omit_empty_values, const std::vector<CommandParserPtr>& commands)
...
/**
 * FormatterProvider for request line.
 */
class RequestLineFormatter : public FormatterProvider {
public:
  RequestLineFormatter();
...
/**
 * FormatterProvider for response line.
 */
class ResponseLineFormatter : public FormatterProvider {
public:
  ResponseLineFormatter();
  // FormatterProvider
...
/**
 * FormatterProvider for node id.
 */
class NodeIdFormatter : public FormatterProvider {
public:
  NodeIdFormatter();
  // FormatterProvider
  absl::optional<std::string> format(const Http::RequestHeaderMap&,
...
/**
 * FormatterProvider for listener stat_prefix.
 */
class ListenerStatPrefixFormatter : public FormatterProvider {
public:
  ListenerStatPrefixFormatter();
  // FormatterProvider
  absl::optional<std::string> format(const Http::RequestHeaderMap&,
                                     const Http::ResponseHeaderMap&,
                                     const Http::ResponseTrailerMap&, const StreamInfo::StreamInfo&,
...
/**
 * FormatterProvider for all request headers.
 */
class AllRequestHeaderFormatter : public FormatterProvider {
public:
  AllRequestHeaderFormatter(bool format_data);
...
/**
 * FormatterProvider for all response headers.
 */
class AllResponseHeaderFormatter : public FormatterProvider {
public:
  AllResponseHeaderFormatter(bool format_data);
...
public:
  ...
  absl::optional<std::string> format(const Http::RequestHeaderMap&, const Http::ResponseHeaderMap&,
                                     const Http::ResponseTrailerMap&, const StreamInfo::StreamInfo&,
                                     absl::string_view) const override;
  const std::string& formatRef(const Http::RequestHeaderMap& request_headers,
                               const Http::ResponseHeaderMap& response_headers,
                               const Http::ResponseTrailerMap& response_trailers,
                               const StreamInfo::StreamInfo& stream_info,
                               absl::string_view local_reply_body) const override;

  bool isSupportFormatRef() const override { return support_format_ref_; }
  ...
  public:
    ...
    virtual const std::string& extractRef(const StreamInfo::StreamInfo&) const {
      return EMPTY_STRING;
    }
...
private:
  bool support_format_ref_{false};
...
public:
  /**
   * 从StreamInfo中的元数据获取"envoy.filters.http.body-to-metadata.1.0"命名空间下跟body相关的数据
   * body相关的数据由L7插件body-to-metadata负责写入元数据中
   * @param stream_info
   * @param key body-to-metadata插件定义如下几个key
   *    request_body_bytes:   请求body的大小(字节数)
   *    response_body_bytes:  响应body的大小(字节数)
   *    request_body: 请求body数据
   *    response_body: 响应body数据
   * @param number metadata中的字段类型是否是数字
   * @return
   */
  static const std::string& getMetadataField(const StreamInfo::StreamInfo& stream_info,
                                             const std::string& key, bool number);
  /**
   * 从StreamInfo中的共享空间获取插件存储的数据
   */
  static const std::string& getSharedData(const StreamInfo::StreamInfo& stream_info,
                                             const std::string& key);
  static bool isPluginHit(const StreamInfo::StreamInfo& stream_info);
  static const std::string& getUniqueId(const StreamInfo::StreamInfo& stream_info);

  inline static const std::string IdenfityVerificationSharedDataUserName{"www.srhino.com.identity-verification.username"};
  inline static const std::string IdenfityVerificationSharedDataToken{"www.srhino.com.identity-verification.token"};
  inline static const std::string UserIdentifySharedDataUserName{"user_identify.user_name"};
  inline static const std::string UserIdentifySharedDataUserUniqueId{"www.srhino.com.user-identify.user_unique_id"};
  inline static const std::string UserIdentifySharedDataAppId{"www.srhino.com.user-identify.app_id"};
  inline static const std::string UserIdentifySharedDataRuleId{"www.srhino.com.user-identify.rule_id"};
  inline static const std::string CommonSharedDataPluginHit{"www.srhino.com.plugin_hit"};
  inline static const std::string LogSharedDataGatewayName{"log.gateway_name"};
  inline static const std::string LogSharedDataVirtualHostName{"log.virtual_host_name"};
  inline static const std::string LogSharedDataClusterName{"log.cluster_name"};
};
...
class SystemTimeInt64Formatter : public SystemTimeFormatter {
public:
  SystemTimeInt64Formatter(const std::string& format, TimeFieldExtractorPtr f);
  ProtobufWkt::Value formatValue(const Http::RequestHeaderMap&, const Http::ResponseHeaderMap&,
                                 const Http::ResponseTrailerMap&, const StreamInfo::StreamInfo&,
                                 absl::string_view) const override;
};
...
/**
 * SystemTimeInt64Formatter (FormatterProvider) for request start time from StreamInfo.
 */
class StartTimeInt64Formatter : public SystemTimeInt64Formatter {
public:
  StartTimeInt64Formatter(const std::string& format);
};
