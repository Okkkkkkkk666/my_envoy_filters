...
public:
  AccessLogManagerImpl(std::chrono::milliseconds file_flush_interval_msec, Api::Api& api,
    ...
    std::string nsq_addr = g_auth_secret->readEnvValue("SCIP");
    nsq_client_ = std::make_shared<NsqClient>(
        nsq_addr.empty() ? nsq_server_address_ : nsq_addr, nsq_server_ports_, topic_, channel_,
        publish_topic_, max_in_flight_, output_buffer_size_, output_buffer_timeout_ms_);
    nsq_loop_thread_ = api_.threadFactory().createThread([this]() -> void { nsqLoopThreadFunc(); },
                                                         Thread::Options{"NSQLoop"});
  }
  ~AccessLogManagerImpl() override;

  void nsqLoopThreadFunc();
  // AccessLog::AccessLogManager
  void reopen() override;
  AccessLogFileSharedPtr createAccessLog(const Filesystem::FilePathAndType& file_info, bool send_to_nsq) override;
...
private:
  ...
  const std::string nsq_server_address_{"sc.srhino.svc.local"};
  const std::vector<int> nsq_server_ports_{5150, 6150, 7150, 8150};
  const std::string topic_{"se_sub_topic"};
  // const std::string topic_{"api_body"};
  const std::string channel_{"se_sub_channel"};
  const std::string publish_topic_{"api_body"};
  // const std::string publish_topic_{"api_body_test"};
  const int max_in_flight_{1};
  const int output_buffer_size_{1024 * 16};
  const int output_buffer_timeout_ms_{200};
  Thread::ThreadPtr nsq_loop_thread_;
  std::shared_ptr<NsqClient> nsq_client_;
};
 ...
 */
class AccessLogFileImpl : public AccessLogFile, Logger::Loggable<Logger::Id::main> {
public:
  AccessLogFileImpl(Filesystem::FilePtr&& file, Event::Dispatcher& dispatcher, bool send_to_nsq,
                    NsqClient& nsq_client, Thread::BasicLockable& lock, AccessLogFileStats& stats,
...
private:
  ...
  void doNsqWrite(std::vector<std::string>& buffer);
  void sendBatchToNsq(std::vector<std::string>& buffer, uint32_t total_size);
  ...
  static const uint64_t MAX_BUFFER_SIZE = 1024 * 1024 * 256;
  ...
  bool send_to_nsq_{false}; // 是否将日志发送到 NSQ
  NsqClient& nsq_client_;
  // 由于默认的OwnedImpl会合并msg，导致使用MPUB发送时，再拆分，效率较低
  std::vector<std::string> nsq_flush_buffer_ ABSL_GUARDED_BY(write_lock_); // This buffer is used by multiple threads. It
  size_t nsq_flush_buffer_len_ ABSL_GUARDED_BY(write_lock_);
  std::vector<std::string> nsq_about_to_write_buffer_; // This buffer is used only by the flush thread. Data
  size_t nsq_about_to_write_buffer_len_;
};
