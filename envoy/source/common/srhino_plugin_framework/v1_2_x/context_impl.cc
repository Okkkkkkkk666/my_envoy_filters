#include "source/common/srhino_plugin_framework/v1_2_x/context_impl.h"
#include "source/common/srhino_plugin_framework/v1_2_x/timer_impl.h"
...
void ContextImpl::setDirectResponse(Utility::HttpCode code, std::unique_ptr<HeaderMap>&& headers,
  ...
  direct_response_.body_str_ = std::move(body);
  direct_response_.body_ = {direct_response_.body_str_.data(), direct_response_.body_str_.size()};
}
...
void ContextImpl::setDirectResponse(Utility::HttpCode code, std::unique_ptr<HeaderMap>&& headers,
  ...
  direct_response_.code_ = code;
  direct_response_.headers_ = std::move(headers);
  direct_response_.body_ = {body.data(), body.size()};
}
...
std::unique_ptr<HeaderMap> ContextImpl::createHeaderMap() const {
  return std::make_unique<HeaderMapImpl>();
}

std::unique_ptr<HeaderMap> ContextImpl::createHeaderMap(
    const std::initializer_list<std::pair<std::string_view, std::string_view>>& headers) const {
  auto result = createHeaderMap();
...
void ContextImpl::directResponseInternal(Utility::HttpCode code,
                                         std::unique_ptr<HeaderMap>&& headers,
                                         absl::string_view body) {
  if (decoder_callbacks_) {
    decoder_callbacks_->sendLocalReply(
        static_cast<Envoy::Http::Code>(code), body,
...
void HeaderContextImpl::setDirectResponse(Utility::HttpCode code,
  ...
  if (is_pause_) {
    ContextImpl::directResponseInternal(code, std::move(headers), {body.data(), body.size()});
  } else {
...
void HeaderContextImpl::setDirectResponse(Utility::HttpCode code,
  ...
  if (is_pause_) {
    ...
    ContextImpl::setDirectResponse(code, std::move(headers), body);
  }
}
