#include "source/common/srhino_plugin_framework/v1_3_x/libs/config/type/matcher/matcher_impl.h"
...
MatcherImpl::MatcherImpl(
    const srhino_plugin_framework::v1_3_x::proto::config::type::matcher::Matcher& matcher)
    : Matcher(matcher), path_matcher_(makePathMatcher(matcher)),
      ...
      referer_matchers_(Envoy::Http::HeaderUtility::buildHeaderDataVector(
          convertProtoConfig(matcher.referers()))),
      ...
      ip_range_(buildIpRangeVector(matcher)), ip_groups_(buildIpGroups(matcher)),
      ip_list_(buildIpSet(matcher)), method_invert_(matcher.method_invert()),
      ip_invert_(matcher.ip_list().invert()), path_invert_(matcher.path_invert()),
      hash_code_(calcHash(matcher)), region_matcher_(buildRegionMatcher(matcher)),
      exclude_path_(buildExcludePath(matcher)), date_matcher_(buildDateMatcher(matcher)) {}
...
MatcherImpl::buildMethodsVector(const v3::Matcher& matcher) const {
...
std::vector<IPGroupsPtr> MatcherImpl::buildIpGroups(const v3::Matcher& matcher) const {
  std::vector<IPGroupsPtr> IPGroups;
  for (const auto& ip_group : matcher.ip_list().ip_groups()) {
    IPGroups.emplace_back(std::make_unique<IPGroupsImpl>(ip_group));
  }
  return IPGroups;
}

RegionMatcherptr MatcherImpl::buildRegionMatcher(const v3::Matcher& matcher) const {
  if (matcher.has_region()) {
    return std::make_unique<RegionMatcherImpl>(matcher.region());
  }
  return nullptr;
}
...
DateMatcherptr MatcherImpl::buildDateMatcher(const v3::Matcher& matcher) const {
  if (matcher.has_date()) {
    return std::make_unique<DateMatcherImpl>(matcher.date());
  }
  return nullptr;
}
...
MatcherImpl::convertProtoConfig(
    const google::protobuf::RepeatedPtrField<
        srhino_plugin_framework::v1_3_x::proto::config::type::matcher::HeaderMatcher>&
        HeaderMatcher) const {
...
MatcherImpl::convertProtoConfig(const SrhinoPluginFramework::v1_3_x::HeaderMap& header_map) const {
...
void MatcherImpl::updateHashWithStringMatcher(
    const srhino_plugin_framework::v1_3_x::proto::config::type::matcher::StringMatcher& matcher,
    uint64_t& hash_code) {
  hashUpdate(matcher.match_pattern_case(), hash_code);
  switch (matcher.match_pattern_case()) {
  case srhino_plugin_framework::v1_3_x::proto::config::type::matcher::StringMatcher::
