...
#include "source/common/srhino_plugin_framework/v1_4_x/libs/net/http_call_impl.h"
#include "source/common/srhino_plugin_framework/v1_4_x/libs/net/grpc_call_impl.h"
...
GrpcCallSharedPtr NetImpl::createGrpcCall(const std::string& cluster_name,
                                          ...
                                          bool is_public_cluster) {
  if (is_public_cluster) {
    return std::make_shared<GrpcCallImpl>(factory_context_, cluster_name, serivce_name, timeout);
  }
  return std::make_shared<GrpcCallImpl>(
      factory_context_,
      std::format("{}.cluster.{}", plugin_file_info_.file_name_, cluster_name, timeout),
      serivce_name);
}
} // namespace Net
} // namespace Libs
