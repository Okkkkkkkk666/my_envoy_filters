...
bool RegexMatcherImpl::init(const std::vector<RuleConfig>& expressions,
                            std::vector<RuleConfig>& failed_exprs) {
  return initWithDataTagRules(expressions, failed_exprs);
}

bool RegexMatcherImpl::initWithDataTagRules(const std::vector<RuleConfig>& expressions,
                                            std::vector<RuleConfig>& failed_exprs) {
  if (HS_SUCCESS != hs_valid_platform()) {
    ENVOY_LOG(warn, "cpu do not support ssse3, use re2 instead");
    return false;
  ...
  for (const auto& rule : expressions) {
    if (rule.expr.empty()) {
      failed_exprs.push_back(rule);
      continue;
    }

    data_tag_rule_list_.push_back(rule);
    if (data_tag_rule_map_.find(rule.id) == data_tag_rule_map_.end()) {
      data_tag_rule_map_[rule.id] = rule;
    } else {
      ENVOY_LOG(warn, "Rule id: {} already exists!", rule.id);
      failed_exprs.push_back(rule);
    }
  }

  // 创建DataTag正则引擎
  HyperscanEngineSharedPtr data_tag_engines = nullptr;
  bool success = false;
  data_tag_engines = createDataTagEngine(failed_exprs);
  if (data_tag_engines) {
    success = true;
  } else {
    ENVOY_LOG(warn, "create data tag regex engine for failed");
  }

  data_tag_engines_ = data_tag_engines;
  return success;
}

HyperscanEngineSharedPtr
RegexMatcherImpl::createDataTagEngine(std::vector<RuleConfig>& failed_exprs) {
  return createDataTagEncodingEngine(EncodingType::UTF8, data_tag_rule_list_, failed_exprs);
}
 ...
 *  举例： 比如正则表达式中含有utf-8字符 "中国",
 * 将其转换成iso-8859-1是失败的，因为iso-8859-1是单字节编码。
 ...
 *        文本的编码为 '82 65 82 66 82 67 82
 * 68'，由于不忽略大小写，hyperscan是按字节一个一个匹配的，导致误匹配。
 *
 */
HyperscanEngineSharedPtr
RegexMatcherImpl::createRegexEngine(EncodingType encode,
                                    std::vector<ExpressionView>& failed_exprs) {
  switch (encode) {
  case EncodingType::ISO_8859_1: {
    ...
    for (const auto& it : expr_list_) {
      ...
      if (RegexUtilities::convertImpl(it->expr(), val, "UTF-8", "ISO-8859-1")) {
        new_exprs.emplace_back(
            std::make_shared<ExpressionView>(val, it->id(), it->ignore_case(), false));
      } else {
...
HyperscanEngineSharedPtr
RegexMatcherImpl::createDataTagEncodingEngine(EncodingType encode,
...
bool RegexMatcherImpl::pcreBlockMatch(const char* data, size_t len,
                                      std::vector<MatchResult>& results, EncodingType encode) {
  return pcreBlockMatch(std::string_view(data, len), results, encode);
}

bool RegexMatcherImpl::pcreBlockMatch(const std::string& data, std::vector<MatchResult>& results,
                                      EncodingType encode) {
  return pcreBlockMatch(data.c_str(), data.length(), results, encode);
}

bool RegexMatcherImpl::pcreBlockMatch(const std::string_view& data,
                                      std::vector<MatchResult>& results, EncodingType encode) {
  if (data.length() == 0) {
    return true;
  }
  ...
  return engines_[encode]->pcreBlockMatch(data, results);
}

bool RegexMatcherImpl::dataTagPcreBlockMatch(const char* data, size_t len,
                                             std::vector<MatchResult>& results, bool left_most) {
  return dataTagPcreBlockMatch(std::string_view(data, len), results, left_most);
}

bool RegexMatcherImpl::dataTagPcreBlockMatch(const std::string& data,
                                             std::vector<MatchResult>& results, bool left_most) {
  return dataTagPcreBlockMatch(data.c_str(), data.length(), results, left_most);
}

bool RegexMatcherImpl::dataTagPcreBlockMatch(const std::string_view& data,
                                             std::vector<MatchResult>& results, bool left_most) {
  if (data.length() == 0) {
    return true;
  }
  ...
  std::shared_lock<std::shared_mutex> lock(mtx_);
  if (!data_tag_engines_) {
    return false;
  }
  return data_tag_engines_->dataTagMatch(data, results, left_most);
}
