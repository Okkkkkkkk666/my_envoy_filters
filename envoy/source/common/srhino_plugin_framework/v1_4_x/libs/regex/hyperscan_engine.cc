...
static thread_local HyperscanEngine::ThreadLocalScanner thread_local_scanners_;
...
bool HyperscanEngine::real_init(const std::vector<ExpressionViewSharedPtr>& expressions,
  ...
  for (size_t i = 0; i < elements; ++i) {
     ...
     */
    if (isPcre(exprs[i])) {
      pcre_pattern_list_.addPattern(expr->expr(), expr->ignore_case(), expr->id());
...
hs_scratch_t* HyperscanEngine::getScratch() {
  ...
  // ENVOY_LOG(debug, "regex {} alloc scratch for thread: {}", reinterpret_cast<uintptr_t>(this),
...
static int dataTagMatchCallback(uint32_t id, uint32_t from, uint32_t to, void* user_data) {
  auto* context = reinterpret_cast<UserData*>(user_data);
  context->results->emplace_back(
      MatchResult{id, static_cast<uint32_t>(from), static_cast<uint32_t>(to)});
  return 0;
}

bool HyperscanEngine::init(const std::vector<RuleConfig>& expressions,
                           [[maybe_unused]] std::vector<RuleConfig>& failed_exprs) {
  if (mode_ != HS_MODE_BLOCK && mode_ != HS_MODE_STREAM && mode_ != HS_MODE_VECTORED) {
    ENVOY_LOG(
        error,
        "mode must have one (and only one) of HS_MODE_BLOCK, HS_MODE_STREAM or HS_MODE_VECTORED");
  ...
  data_tag_db_ = std::make_shared<DataTagDatabase>(expressions);
  return true;
}
...
void HyperscanEngine::blockScan(const std::string_view& data, std::vector<MatchResult>& results) {
  if (!db_)
    return;
  ...
  Pcre* pcre = getThreadLocalPcre();
  UserData user_data{&results, pcre, data, &block_match_res};

  hs_scratch_t* scratch = getScratch();
  hs_error_t err =
      hs_scan(db_, data.data(), data.length(), 0, scratch, blockScanCallback, &user_data);
  if (err != HS_SUCCESS) {
    return;
  }

  for (const auto& [real_id, match_list] : block_match_res) {
    const auto* pattern = pcre->getPattern(real_id);
    if (pattern) {
      // 再次用 PCRE 扫描匹配区域
      pcreScan(real_id, pattern, match_list, results, data, pcre);
    } else {
      for (const auto& [from, to] : match_list) {
        results.emplace_back(MatchResult{real_id, from, to});
      }
    }
  }
}

int HyperscanEngine::blockScanCallback(unsigned int id, unsigned long long from,
                                       unsigned long long to, unsigned int, void* user_data) {
  auto* context = reinterpret_cast<UserData*>(user_data);

  if (!context) {
    return -1;
  }
  auto iter = context->block_match_res->find(id);
  if (iter != context->block_match_res->end()) {
    auto& list = iter->second;
    auto& back = list.back();
    if (back.first == from) {
      if (likely(to > back.second)) {
        auto diff = to - back.second;
        if (diff == 1 || diff == 3) {
          back.second = to;
        } else {
    ...
    list.emplace_back(from, to);
    context->block_match_res->insert({id, std::move(list)});
  }
  return 0;
}

void HyperscanEngine::pcreScan(uint32_t id, const PcrePattern* pattern,
                               const std::list<std::pair<uint32_t, uint32_t>>& hs_results,
                               std::vector<MatchResult>& results, const std::string_view& data,
                               Pcre* pcre) {
  ...
  std::set<uint32_t> unique_to;
  for (auto pos = hs_results.cbegin(); !finish && pos != hs_results.cend(); ++pos) {
    pcre_scan_from =
        pos->second > max_pcre_scan_front_len ? pos->second - max_pcre_scan_front_len : 0;
    pcre_scan_to = pos->second + max_pcre_scan_back_len < data.length()
                       ? pos->second + max_pcre_scan_back_len
                       : data.length();
    auto next = std::next(pos);
    while (next != hs_results.cend()) {
      if (pcre_scan_to - pcre_scan_from < max_pcre_scan_len && pcre_scan_to > next->second) {
        ++pos;
        ++next;
        if (pos->second + max_pcre_scan_back_len < data.length()) {
          pcre_scan_to = pos->second + max_pcre_scan_back_len;
        } else {
          pcre_scan_to = data.length();
          finish = true;
          break;
        }
      } else {
    ...
    std::string_view pcre_scan_data = data.substr(pcre_scan_from, pcre_scan_to - pcre_scan_from);
    auto pcre_match_info = pcre->matchGlobal(pattern, pcre_scan_data);
    for (const auto& from_to : pcre_match_info) {
      ...
      if (unique_to.find(to) != unique_to.end()) {
        continue;
      }
      unique_to.insert(to);
      results.emplace_back(MatchResult{id, static_cast<uint32_t>(from), static_cast<uint32_t>(to)});
    }
  }
}
...
int HyperscanEngine::matchCallback(unsigned int id, unsigned long long from, unsigned long long to,
  ...
  if (unlikely(pcre_pattern)) {
    ...
    if (match_global) {
      auto pcre_match_info = parent->pcre->matchGlobal(pcre_pattern, pcre_scan_data);
      for (const auto& from_to : pcre_match_info) {
        // init from and to
        const size_t ovector_from = from_to.first;
        const size_t ovector_to = from_to.second;
...
bool HyperscanEngine::pcreBlockMatch(const std::string_view& data,
                                     std::vector<MatchResult>& results) {
  blockScan(data, results);
  // 贪婪模式，对重复匹配进行压缩
  return greedyProcess(results);
}

bool HyperscanEngine::dataTagMatch(const std::string_view& data, std::vector<MatchResult>& results,
                                   bool left_most) {
  auto scanner = thread_local_scanners_.getScanner(data_tag_db_);
  UserData user_data{
      &results,
      NULL,
      data,
  ...
  scanner->registMatchCallback(dataTagMatchCallback, &user_data);
  if (!scanner->scan(data, left_most)) {
    ENVOY_LOG(error, "DataTagScanner scan failed");
    return false;
  }
  return greedyProcess(results);
}
