...
ConnPoolImplBase::ConnPoolImplBase(
    ...
    : state_(state), host_(host), priority_(priority), dispatcher_(dispatcher),
      ...
      stream_flags_(dispatcher_.streamFlags()) {}
...
void ConnPoolImplBase::attachStreamToClient(Envoy::ConnectionPool::ActiveClient& client,
  ...
  if (!(stream_flags_ & Event::Dispatcher::StreamFlag::Nuclei)) {
    host_->cluster().stats().upstream_rq_total_.inc();
    auto& host_rq_total_counter = host_->cluster().statsScope(true).gaugeFromString(
        Http::CodeStatsImpl::makeHostResponseStatName(host_, "upstream_rq_total", true),
        Stats::Gauge::ImportMode::NeverImport);
    host_rq_total_counter.inc();
  }
...
void ConnPoolImplBase::onStreamClosed(Envoy::ConnectionPool::ActiveClient& client,
  ...
  num_active_streams_--;
  if (host_->stats().rq_active_.value() > 0) {
    host_->stats().rq_active_.dec();
  }
  if (host_->cluster().stats().upstream_rq_active_.value() > 0) {
...
ActiveClient::ActiveClient(ConnPoolImplBase& parent, uint32_t lifetime_stream_limit,
  ...
  if (!(parent_.dispatcher().streamFlags() & Event::Dispatcher::StreamFlag::Nuclei)) {
    parent_.host()->stats().cx_total_.inc();
    parent_.host()->cluster().stats().upstream_cx_total_.inc();

    parent_.host()->cluster().statsScope(true).gaugeFromString(
        Http::CodeStatsImpl::makeHostResponseStatName(parent_.host(), "upstream_cx_total", true),
        Stats::Gauge::ImportMode::NeverImport).inc();
  }
  parent_.host()->stats().cx_active_.inc();
  parent_.host()->cluster().stats().upstream_cx_active_.inc();

  // 由于DNS有可能多个IP，导致parent_.host()在ActiveClient构造时跟releaseResourcesBase时有可能不是同一对象
  // 所以需用成员变量保存Gauge对象，避免在releaseResourcesBase中使用makeHostResponseStatName构造不同的Gauge
  // 另外，连接数是所有Stream共享的，所以不能使用Event::Dispatcher::StreamFlag::Nuclei来过滤，否则影响其他Stream
  host_cx_active_counter_ = &(parent_.host()->cluster().statsScope(true).gaugeFromString(
      Http::CodeStatsImpl::makeHostResponseStatName(parent_.host(), "upstream_cx_active", true),
      Stats::Gauge::ImportMode::NeverImport));
  host_cx_active_counter_->inc();
...
void ActiveClient::releaseResourcesBase() {
  if (!resources_released_) {
    ...
    if (host_cx_active_counter_) {
      host_cx_active_counter_->dec();
    }
