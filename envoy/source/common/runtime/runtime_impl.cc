...
LoaderImpl::LoaderImpl(Event::Dispatcher& dispatcher, ThreadLocal::SlotAllocator& tls,
    ...
    : generator_(generator), stats_(generateStats(store)), tls_(tls.allocateSlot()),
      config_(config), service_cluster_(local_info.clusterName()), api_(api), admin_(admin),
...
void RtdsSubscription::onConfigUpdate(const std::vector<Config::DecodedResourceRef>& resources,
  ...
  // 加载/卸载so文件
  ThreadSafeSingleton<SrhinoPluginFramework::PluginFileManager>::get().init(parent_.cm_,
                                                                           &parent_.admin_);
  const auto& layers = parent_.snapshot().getLayers();
  for (auto& layer : layers) {
    if (layer->name() == "srhino_plugins_layer") {
      std::set<std::string> files;
      for (const auto& value : layer->values()) {
        files.emplace(value.first);
      }
      ThreadSafeSingleton<SrhinoPluginFramework::PluginFileManager>::get().merge(files);
      break;
    }
  }
}
