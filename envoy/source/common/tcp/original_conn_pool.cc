...
void OriginalConnPoolImpl::assignConnection(ActiveConn& conn,
  ...
  conn.wrapper_ = std::make_shared<ConnectionWrapper>(conn, dispatcher_);
...
OriginalConnPoolImpl::ConnectionWrapper::ConnectionWrapper(ActiveConn& parent,
                                                           Event::Dispatcher& dispatcher)
    : parent_(parent) {
  if (!(dispatcher.streamFlags() & Event::Dispatcher::StreamFlag::Nuclei)) {
    parent_.parent_.host_->cluster().stats().upstream_rq_total_.inc();
    auto& host_rq_total_counter = parent_.parent_.host_->cluster().statsScope(true).gaugeFromString(
        Http::CodeStatsImpl::makeHostResponseStatName(parent_.parent_.host(), "upstream_rq_total",
                                                      true),
...
void OriginalConnPoolImpl::ConnectionWrapper::release(bool closed) {
  ...
  if (!released_) {
    ...
    if (parent_.parent_.host_->cluster().stats().upstream_rq_active_.value() > 0) {
      parent_.parent_.host_->cluster().stats().upstream_rq_active_.dec();
    }

    if (parent_.parent_.host_->stats().rq_active_.value() > 0) {
...
OriginalConnPoolImpl::ActiveConn::ActiveConn(OriginalConnPoolImpl& parent)
  ...
  auto& host_cx_active_counter = parent_.host_->cluster().statsScope(true).gaugeFromString(
      Http::CodeStatsImpl::makeHostResponseStatName(parent_.host(), "upstream_cx_active", true),
      Stats::Gauge::ImportMode::NeverImport);
  host_cx_active_counter.inc();
