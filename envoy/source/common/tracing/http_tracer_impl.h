...
public:
  ...
  const std::set<std::string>& getSpanTagsFromTracing() const override {
    static std::set<std::string> unuse_set;
    return unuse_set;
  }
  bool hasSpanTag(const std::string& tag) const override {
    std::string unuse_str = tag;
    return false;
  }
};
