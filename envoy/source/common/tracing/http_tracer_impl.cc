...
void HttpTracerUtility::finalizeDownstreamSpan(Span& span,
  ...
  if (request_headers) {
    ...
    if (tracing_config.hasSpanTag(Tracing::Tags::get().PeerAddress)) {
    ...
    if (request_headers->ClientTraceId()) {
      if (tracing_config.hasSpanTag(Tracing::Tags::get().GuidXClientTraceId)) {
  ...
  if (response_headers) {
    if (tracing_config.hasSpanTag(Tracing::Tags::get().ContentType)) {
      const Http::HeaderEntry* content_type = response_headers->ContentType();
      if (content_type != nullptr) {
        span.setTag(Tracing::Tags::get().ContentType, content_type->value().getStringView());
      } else {
        span.setTag(Tracing::Tags::get().ContentType, "");
      }
    }
  }
  ...
  if (tracing_config.hasSpanTag(Tracing::Tags::get().RequestSize)) {
    span.setTag(Tracing::Tags::get().RequestSize, std::to_string(stream_info.bytesReceived()));
  }
  if (tracing_config.hasSpanTag(Tracing::Tags::get().ResponseSize)) {
...
void HttpTracerUtility::finalizeUpstreamSpan(Span& span,
  ...
  if (stream_info.upstreamInfo() && stream_info.upstreamInfo()->upstreamHost() &&
      tracing_config.hasSpanTag(Tracing::Tags::get().UpstreamAddress)) {
...
void HttpTracerUtility::setCommonTags(Span& span, const Http::ResponseHeaderMap* response_headers,
                                      ...
                                      const StreamInfo::StreamInfo& stream_info,
                                      const Config& tracing_config) {
  if (tracing_config.hasSpanTag(Tracing::Tags::get().Component)) {
    span.setTag(Tracing::Tags::get().Component, Tracing::Tags::get().Proxy);
  }
  if (stream_info.upstreamInfo() && stream_info.upstreamInfo()->upstreamHost()) {
    if (tracing_config.hasSpanTag(Tracing::Tags::get().UpstreamCluster)) {
      span.setTag(Tracing::Tags::get().UpstreamCluster,
                  stream_info.upstreamInfo()->upstreamHost()->cluster().name());
    }
    if (tracing_config.hasSpanTag(Tracing::Tags::get().UpstreamClusterName)) {
  ...
  // Post response data.
  if (tracing_config.hasSpanTag(Tracing::Tags::get().HttpStatusCode)) {
    span.setTag(Tracing::Tags::get().HttpStatusCode, buildResponseCode(stream_info));
  }
  if (tracing_config.hasSpanTag(Tracing::Tags::get().ResponseFlags)) {
...
SpanPtr HttpTracerImpl::startSpan(const Config& config, Http::RequestHeaderMap& request_headers,
  ...
  if (active_span) {
    if (config.hasSpanTag(Tracing::Tags::get().NodeId)) {
      active_span->setTag(Tracing::Tags::get().NodeId, local_info_.nodeName());
    }
    if (config.hasSpanTag(Tracing::Tags::get().Zone)) {
      active_span->setTag(Tracing::Tags::get().Zone, local_info_.zoneName());
    }
    if (config.hasSpanTag(Tracing::Tags::get().Sni)) {
      auto it = local_info_.node().metadata().fields().find("sni");
      if (it != local_info_.node().metadata().fields().end()) {
        active_span->setTag(Tracing::Tags::get().Sni,
                            local_info_.node().metadata().fields().at("sni").string_value());
      } else {
        active_span->setTag(Tracing::Tags::get().Sni, "");
      }
    }
