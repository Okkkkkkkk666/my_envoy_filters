...
struct StreamInfoImpl : public StreamInfo {
  ...
  void setListenerStatPrefix(const std::string& listener_stat_prefix) override {
    listener_stat_prefix_ = listener_stat_prefix;
  }
  const std::string& listenerStatPrefix() const override { return listener_stat_prefix_; }

  const Network::Address::InstanceConstSharedPtr& predictUpstreamRemoteAddress() const override {
  ...
  void setIsLocalReply(bool is_local_reply) override { is_local_reply_ = is_local_reply; }
  bool isLocalReply() const override { return is_local_reply_; }

  void setOriginHost(absl::string_view host) override { origin_host_ = std::string(host); }
  const std::string& originHost() const override { return origin_host_; }

  void setIsVHostMatched(bool matched) override { is_vhost_matched_ = matched; }
  bool isVHostMatched() const override { return is_vhost_matched_; }
...
private:
  ...
  std::string listener_stat_prefix_;
  Network::Address::InstanceConstSharedPtr predict_upstream_remote_address_{};
  // 当因为本地回复等原因，请求被中断，日志中不包含业务地址，设置此标志，在streamComplete处执行特殊处理
  // 当本地回复时，回复数据不经过动态插件链
  bool is_local_reply_{false};
  // 原始请求host
  std::string origin_host_{};
  // 是否匹配到的虚拟服务中的 host
  bool is_vhost_matched_{false};
};
