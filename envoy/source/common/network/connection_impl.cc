...
ConnectionImpl::ConnectionImpl(Event::Dispatcher& dispatcher, ConnectionSocketPtr&& socket,
    ...
    : ConnectionImplBase(dispatcher, next_global_id_++),
      ...
      transport_wants_read_(false), stream_flags_(dispatcher_.streamFlags()) {
...
void ConnectionImpl::updateReadBufferStats(uint64_t num_read, uint64_t new_size) {
  ...
  if (host_rq_gauge_) {
    std::string remote_addr =
        socket_->connectionInfoProviderSharedPtr()->remoteAddress()->asString();
    std::string host_rq_gauge_name = host_rq_gauge_->name();
    std::string::size_type idx = host_rq_gauge_name.find(remote_addr);
    // gauge名称中能找到conn所属地址,表示当前conn所属地址是当前gauge所统计的主机
    if (idx != std::string::npos) {
      host_rq_gauge_->add(num_read);
    }
  }
}
...
void ConnectionImpl::updateWriteBufferStats(uint64_t num_written, uint64_t new_size) {
  ...
  if (host_rs_gauge_) {
    std::string remote_addr =
        socket_->connectionInfoProviderSharedPtr()->remoteAddress()->asString();
    std::string host_rs_gauge_name = host_rs_gauge_->name();
    std::string::size_type idx = host_rs_gauge_name.find(remote_addr);
    // gauge名称中能找到conn所属地址,表示当前conn所属地址是当前gauge所统计的主机
    if (idx != std::string::npos) {
      host_rs_gauge_->add(num_written);
    }
  }
}
