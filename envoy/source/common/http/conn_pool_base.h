...
public:
  ...
  void initialize(Upstream::Host::CreateConnectionData& data, HttpConnPoolImplBase& parent) {
    ...
    // 单机级别上行总量
    auto& host_rq_size_gauge = parent_.host()->cluster().statsScope(true).gaugeFromString(
        Http::CodeStatsImpl::makeHostResponseStatName(parent_.host(), "upstream_rq_size", true),
        Stats::Gauge::ImportMode::NeverImport);

    // 单机级别下行总量
    auto& host_rs_size_gauge = parent_.host()->cluster().statsScope(true).gaugeFromString(
        Http::CodeStatsImpl::makeHostResponseStatName(parent_.host(), "upstream_rs_size", true),
        Stats::Gauge::ImportMode::NeverImport);
    codec_client_->setHostGauge(host_rq_size_gauge, host_rs_size_gauge);
