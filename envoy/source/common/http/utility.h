...
void UpdateXff(RequestHeaderMap& headers, const Network::Address::Instance& remote_address);
 ...
 * @param shift specifies the direction from left to right when set true.
...
GetLastAddressFromXffInfo getLastAddressFromXFF(const Http::RequestHeaderMap& request_headers,
                                                uint32_t num_to_skip = 0, bool shift = false);
