...
void Utility::UpdateXff(RequestHeaderMap& headers,
                        const Network::Address::Instance& remote_address) {
  if (remote_address.type() != Network::Address::Type::Ip) {
    return;
  }
  ...
  if (!headers.ForwardedFor()) {
    headers.setReferenceForwardedFor(remote_address.ip()->addressAsString());
  } else {
    headers.setForwardedFor(remote_address.ip()->addressAsString());
  }
}
...
Utility::getLastAddressFromXFF(const Http::RequestHeaderMap& request_headers, uint32_t num_to_skip,
  ...
  if (!shift) {
    ...
    for (uint8_t i = 0; i < search_time; i++) {
      last_comma = xff_string.rfind(separator);
      if (last_comma == std::string::npos) {
        // return {nullptr, false};
        break;
      }
      if (last_comma != std::string::npos && last_comma + separator.size() < xff_string.size()) {
        absl::string_view cur_xff_string = xff_string.substr(last_comma + separator.size());
        if (Network::Utility::isInternalAddress(std::string(cur_xff_string))) {
          xff_string = cur_xff_string;
          break;
        }
        xff_string = xff_string.substr(0, last_comma);
      }
    ...
    // Ignore the last num_to_skip addresses at the start of XFF.
    for (uint32_t i = 0; i < num_to_skip; i++) {
      const std::string::size_type last_comma = xff_string.find(separator);
      // if no enough ip to skip, use the last one
      if (last_comma == std::string::npos) {
        break;
      }
    }
    for (uint8_t i = 0; i < search_time; i++) {
      // The text after the last remaining comma, or the entirety of the string if there
      // is no comma, is the requested IP address.
      last_comma = xff_string.find(separator);
      if (last_comma == std::string::npos) {
        break;
      }
      if (last_comma != std::string::npos && last_comma > 0) {
        absl::string_view cur_xff_string = xff_string.substr(0, last_comma);
        if (Network::Utility::isInternalAddress(std::string(cur_xff_string))) {
          xff_string = cur_xff_string;
          break;
        }
        xff_string = xff_string.substr(last_comma + 1);
      }
    }
  }
