...
void CodeStatsImpl::incGauge(Stats::Scope& scope, const std::string& name) const {
  scope.gaugeFromString(name, Stats::Gauge::ImportMode::NeverImport).inc();
}
...
void CodeStatsImpl::chargeBasicResponseStat(
    Stats::Scope& scope, Stats::StatName prefix, Code response_code, bool exclude_http_code_stats,
  ...
  if (!exclude_http_code_stats) {
    ...
    if (!rq_group.empty()) {
      ...
      // 单机指标
      if (upstream_host) {
        uint32_t group = enumToInt(response_code) / 100;
        if (group >= 1 && group <= 5) {
          std::string stat_name = fmt::format("upstream_rq_{}xx", group);
          incGauge(upstream_host->cluster().statsScope(true),
                   makeHostResponseStatName(upstream_host, stat_name, true));
        }
      }
    }
...
void CodeStatsImpl::chargeResponseStat(
    const ResponseStatInfo& info, bool exclude_http_code_stats,
    Upstream::HostDescriptionConstSharedPtr upstream_host) const {
...
std::string
CodeStatsImpl::makeHostResponseStatName(Upstream::HostDescriptionConstSharedPtr upstream_host,
                                        std::string_view stat_name, bool global) {
  std::string host_name(upstream_host->hostname());
  if (host_name.empty()) {
    host_name = upstream_host->address()->ip()->addressAsString();
  } else {
    if (host_name.find(":") == std::string::npos) {
      host_name += ":";
      host_name += std::to_string(upstream_host->address()->ip()->port());
    }
  }

  if (global) {
    return fmt::format("cluster.{}.[{}].{}.{}", upstream_host->cluster().name(), host_name,
                       upstream_host->address()->asString(), stat_name);
  }

  return fmt::format("[{}].{}.{}", host_name, upstream_host->address()->asString(), stat_name);
}
