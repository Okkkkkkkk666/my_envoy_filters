...
const std::set<std::string>& ConnectionManagerImpl::ActiveStream::getSpanTagsFromTracing() const {
  return connection_manager_.config_.tracingConfig()->span_tags_;
}

bool ConnectionManagerImpl::ActiveStream::hasSpanTag(const std::string& tag) const {
  const std::set<std::string>& span_tags = getSpanTagsFromTracing();
  if (span_tags.empty()) {
    return true;
  }
  std::set<std::string>::const_iterator it = span_tags.find(tag);
  if (it == span_tags.end()) {
    return false;
  }
  return true;
