...
ConnectionManagerUtility::MutateRequestHeadersResult ConnectionManagerUtility::mutateRequestHeaders(
  ...
  if (config.useRemoteAddress()) {
  ...
  } else if (config.useXRealIp()) {
    const Envoy::Http::LowerCaseString x_real_ip_key("x-real-ip");
    auto x_real_ip = request_headers.get(x_real_ip_key);
    if (x_real_ip.size() == 1) {
      auto header_value = x_real_ip[0]->value().getStringView();
      final_remote_address = Network::Utility::parseInternetAddressNoThrow(std::string(header_value));
    }
  } else {
    ...
    OriginalIPDetectionParams params = {request_headers,
                                        connection.connectionInfoProvider().remoteAddress(), config.xffTrustedHopsShift()};
  ...
  if (!config.skipXffAppend()) {
    ...
    if (Network::Utility::isLoopbackAddress(
            *connection.connectionInfoProvider().remoteAddress())) {
      remote_address = &config.localAddress();
    } else {
      remote_address = connection.connectionInfoProvider().remoteAddress().get();
    }
    if (config.xffReplace()) {
      Utility::UpdateXff(request_headers, *remote_address);
    } else {
