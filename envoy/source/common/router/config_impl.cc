...
RouteEntryImplBase::RouteEntryImplBase(const VirtualHostImpl& vhost,
    ...
    : case_sensitive_(PROTOBUF_GET_WRAPPED_OR_DEFAULT(route.match(), case_sensitive, true)),
      ...
      // rewrite_query_(route.route().rewrite_query()),
  ...
  // TODO:临时办法，后续应当增加proto字段
  if (route.route().prefix_rewrite() == "rewrite param") {
    rewrite_query_ = true;
  } else {
    prefix_rewrite_ = route.route().prefix_rewrite();
  }
  ...
  if (!rewrite.empty() || regex_rewrite_ != nullptr) {
    ...
    if (regex_rewrite_ != nullptr) {
      if (!rewrite_query_) {
        ...
        return regex_rewrite_->replaceAll(path, regex_rewrite_substitution_);
      }
    }
  }
...
RouteConstSharedPtr RouteMatcher::route(const RouteCallback& cb,
  ...
  const_cast<StreamInfo::StreamInfo*>(&stream_info)->setOriginHost(headers.getHostValue());
  if (virtual_host) {
    // 前端已经限制配置虚拟主机时，必须配置域名和ip，因此是全匹配，所以这里使用headers中的host即可
    const_cast<StreamInfo::StreamInfo*>(&stream_info)->setIsVHostMatched(true);
...
PerFilterConfigs::PerFilterConfigs(
  ...
  for (const auto& it : typed_configs) {
    ...
    static const std::string srhino_plugin_loader_name("envoy.filters.http.srhino-plugin-loader.1.0");

    auto pos = name.find(srhino_plugin_loader_name);
    if (pos != std::string::npos) {
      std::string filter_instance_name = name.substr(pos + srhino_plugin_loader_name.size() + 1);
      auto object = createRouteSpecificFilterConfig(srhino_plugin_loader_name, it.second, optional_http_filters,
                                                    factory_context, validator);
      if (object != nullptr) {
        srhino_plugin_configs_[filter_instance_name] = std::move(object);
      }
      // 兼容使用1.0,1.1,1.2框架的插件
      envoy::extensions::filters::http::srhino_plugin_loader::v3::SrhinoPluginLoaderPerRoute per_route;
      if (it.second.UnpackTo(&per_route)) {
        std::string plugin_file_name = per_route.plugin_file_name();
        auto route_config = per_route.config();
        std::string compatible_filter_instance_name = "compatible_" + filter_instance_name;
        ENVOY_LOG(debug, "compatible plugin {}", plugin_file_name);
        RouteSpecificFilterConfigConstSharedPtr config =
            std::dynamic_pointer_cast<RouteSpecificFilterConfig>(
                std::make_shared<SrhinoPluginFramework::ConfigPerRouteImpl>(std::move(route_config)));
        srhino_plugin_configs_[compatible_filter_instance_name] = std::move(config);
      }
    } else if (!SrhinoPluginFramework::FileNameMatcher::matchInstanceName(name)) {
      ...
      if (it.second.UnpackTo(&message)) {
        RouteSpecificFilterConfigConstSharedPtr config =
            std::dynamic_pointer_cast<RouteSpecificFilterConfig>(
                std::make_shared<SrhinoPluginFramework::ConfigPerRouteImpl>(std::move(message)));
        srhino_plugin_configs_[name] = std::move(config);
      }
    }
  }
}
...
const RouteSpecificFilterConfig* PerFilterConfigs::get(const std::string& name) const {
  auto it = srhino_plugin_configs_.find(name);
  if (it != srhino_plugin_configs_.end()) {
