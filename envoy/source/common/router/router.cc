...
Filter::~Filter() {
  ...
  callbacks_->dispatcher().clearStreamFlags(Envoy::Event::Dispatcher::StreamFlag::Nuclei);
}
...
void Filter::chargeUpstreamCode(uint64_t response_status_code,
  ...
  if (config_.emit_dynamic_stats_ && !callbacks_->streamInfo().healthCheck()) {
    ...
    code_stats.chargeResponseStat(info, exclude_http_code_stats_, upstream_host);
    ...
    if (alt_stat_prefix_ != nullptr) {
      Http::CodeStats::ResponseStatInfo alt_info{config_.scope_,
                                                 ...
                                                 upstream_zone,
                                                 is_canary};
      code_stats.chargeResponseStat(alt_info, exclude_http_code_stats_, upstream_host);
    }
...
Http::FilterHeadersStatus Filter::decodeHeaders(Http::RequestHeaderMap& headers, bool end_stream) {
  ...
  if ((headers.getUserAgentValue() == "nuclei") || (headers.getUserAgentValue() == "HAHC")) {
    is_nuclei_ = true;
  }
  ...
  if (is_nuclei_) {
    callbacks_->dispatcher().setStreamFlags(Envoy::Event::Dispatcher::StreamFlag::Nuclei);
  } else {
    callbacks_->dispatcher().clearStreamFlags(Envoy::Event::Dispatcher::StreamFlag::Nuclei);
  }
...
void Filter::sendNoHealthyUpstreamResponse() {
  ...
  std::string code_details =
      config_.runtime_.snapshot().get("NoHealthyUpstream").has_value()
          ? config_.runtime_.snapshot().get("NoHealthyUpstream").value().get()
          : "no healthy upstream";

  callbacks_->sendLocalReply(
      Http::Code::ServiceUnavailable, code_details,
      [this](Http::ResponseHeaderMap& headers) {
        headers.setContentType("text/plain;charset=UTF-8");
      },
      absl::nullopt, StreamInfo::ResponseCodeDetails::get().NoHealthyUpstream);
}
