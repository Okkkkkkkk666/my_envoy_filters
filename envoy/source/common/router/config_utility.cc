...
ConfigUtility::QueryParameterMatcher::QueryParameterMatcher(
    ...
    : name_(config.name()), string_matcher_(maybeCreateStringMatcher(config)) {

  if (config.query_parameter_match_specifier_case() ==
      envoy::config::route::v3::QueryParameterMatcher::QueryParameterMatchSpecifierCase::
          kPresentMatch) {
...
bool ConfigUtility::QueryParameterMatcher::matches(
  ...
  bool param_exists = query_param != request_query_params.end();

  if (present_match_.has_value()) {
    return present_match_.value() ? param_exists : !param_exists;
  }

  if (!param_exists) {
    return false;
  }
  return string_matcher_.has_value() ? string_matcher_.value().match(query_param->second) : false;
}
