...
public:
  HostDescriptionImpl(
      ...
      const std::string& dns_addr = Envoy::EMPTY_STRING);
  ...
  const std::string& dnsAddr() const override { return dns_addr_; }
...
public:
  HostImpl(ClusterInfoConstSharedPtr cluster, const std::string& hostname,
           ...
           TimeSource& time_source, const std::string& dns_addr = Envoy::EMPTY_STRING)
      : HostDescriptionImpl(cluster, hostname, address, metadata, locality, health_check_config,
                            priority, time_source, dns_addr),
        used_(true) {
  ...
  void healthFailReasonSet(HealthFailReason reason) final {
    if (HostImpl::health_fail_reason_prior_.at(reason) <
        HostImpl::health_fail_reason_prior_.at(health_fail_reason_)) {
      health_fail_reason_ = reason;
    }
  }
  Host::HealthFailReason healthFailReasonGet() const override { return health_fail_reason_; }
  void healthFailReasonClear() final { health_fail_reason_ = Host::HealthFailReason::NONE; }
...
private:
  ...
  HealthFailReason health_fail_reason_{};
  static const std::unordered_map<HealthFailReason, int> health_fail_reason_prior_;
};
...
public:
  ...
  Stats::Scope& statsScope(bool global = false) const override {
    return global ? factory_context_->serverScope() : *stats_scope_;
  }
