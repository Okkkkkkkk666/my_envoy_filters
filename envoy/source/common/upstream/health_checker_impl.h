  ...
  struct HttpActiveHealthCheckSession : public ActiveHealthCheckSession,
    ...
    void decodeData(Buffer::Instance& data, bool end_stream) override {
      ...
      if (response_code != 200) {
        response_data_.append(data.toString());
      }
      if (end_stream) {
        onResponseComplete();
      }
...
/**
 * PING health checker implementation.
 */
class PingHealthCheckerImpl : public HealthCheckerImplBase {
public:
  PingHealthCheckerImpl(const Cluster& cluster, const envoy::config::core::v3::HealthCheck& config,
                       Event::Dispatcher& dispatcher, Runtime::Loader& runtime,
                       Random::RandomGenerator& random, HealthCheckEventLoggerPtr&& event_logger);

private:
  struct PingActiveHealthCheckSession : public ActiveHealthCheckSession {
    PingActiveHealthCheckSession(PingHealthCheckerImpl& parent, const HostSharedPtr& host)
        : ActiveHealthCheckSession(parent, host), parent_(parent),
          icmp_polling_interval_(parent_.timeout() <= std::chrono::milliseconds(50) ? std::chrono::milliseconds(10) : std::chrono::milliseconds(50)),
          polling_timer_(parent.dispatcher_.createTimer([this]() -> void { onPollingTimer(); })) {}
    ~PingActiveHealthCheckSession() override;
    ...
    PingHealthCheckerImpl& parent_;
    struct libicmp *icmp_client_ = {nullptr};
    const std::chrono::milliseconds icmp_polling_interval_;
    Event::TimerPtr polling_timer_;
  ...
  using PingActiveHealthCheckSessionPtr = std::unique_ptr<PingActiveHealthCheckSession>;

  // HealthCheckerImplBase
  ActiveHealthCheckSessionPtr makeSession(HostSharedPtr host) override {
    return std::make_unique<PingActiveHealthCheckSession>(*this, host);
  }
  envoy::data::core::v3::HealthCheckerType healthCheckerType() const override {
    return envoy::data::core::v3::TCP;
  }
