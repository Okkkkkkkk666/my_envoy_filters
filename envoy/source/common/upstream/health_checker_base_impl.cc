...
HealthTransition HealthCheckerImplBase::ActiveHealthCheckSession::setUnhealthy(
  ...
  if (networkHealthCheckFailureType(type)) {
    if (host_->healthFailReasonGet() == Host::HealthFailReason::NONE) {
      host_->healthFailReasonSet(Host::HealthFailReason::CONNECT_FAILED);
    }
  }
  ...
  if (!host_->healthFlagGet(Host::HealthFlag::FAILED_ACTIVE_HC)) {
    ...
    // 当host此前为健康，经过xds更新后，该host拥有Host::HealthFlag::FAILED_ACTIVE_HC状态，此时host永远陷入健康状态
    // 为避免这种情况，需在host拥有Host::HealthFlag::FAILED_ACTIVE_HC状态时更新健康状态
    setHealthyFlagToStats();
  }
...
void HealthCheckerImplBase::ActiveHealthCheckSession::setHealthyFlagToStats() const {
  envoy::config::core::v3::HealthStatus health_status =
      envoy::config::core::v3::HealthStatus::UNKNOWN;
  Host::HealthFailReason health_fail_reason = Host::HealthFailReason::NONE;
  switch (host_->health()) {
  case Host::Health::Healthy:
    health_status = envoy::config::core::v3::HealthStatus::HEALTHY;
    break;
  case Host::Health::Unhealthy:
    if (host_->healthFlagGet(Host::HealthFlag::ACTIVE_HC_TIMEOUT)) {
      health_status = envoy::config::core::v3::HealthStatus::TIMEOUT;
    } else {
      health_status = envoy::config::core::v3::HealthStatus::UNHEALTHY;
    }
    health_fail_reason = host_->healthFailReasonGet();
    break;
  case Host::Health::Degraded:
    health_status = envoy::config::core::v3::HealthStatus::DEGRADED;
    break;
  default:
  ...
  // 高位表示原因，低位表示状态
  uint64_t status = static_cast<uint64_t>(health_fail_reason);
  status <<= 8;
  status += static_cast<uint64_t>(health_status);
  auto& health_status_gauge = parent_.cluster_.info()->statsScope(false).gaugeFromString(
      Http::CodeStatsImpl::makeHostResponseStatName(host_, "health_check.health_status", false),
      Stats::Gauge::ImportMode::NeverImport);
  health_status_gauge.set(status);
}
