...
HealthCheckerSharedPtr HealthCheckerFactory::create(
  ...
  case envoy::config::core::v3::HealthCheck::HealthCheckerCase::kTcpHealthCheck:
    if (health_check_config.tcp_health_check().send().text() == "sr-ping") {
      return std::make_shared<PingHealthCheckerImpl>(cluster, health_check_config, dispatcher,
                                                     runtime, api.randomGenerator(),
                                                     std::move(event_logger));
    } else {
      return std::make_shared<TcpHealthCheckerImpl>(cluster, health_check_config, dispatcher,
                                                    runtime, api.randomGenerator(),
                                                    std::move(event_logger));
    }
...
/**
 * 本函数实现一种简单的模糊搜索，在data中搜索patterns中定义的字符串，根据匹配成功百分比确定最终的匹配结果
 * @param
 * data 要匹配的文本数据
 * patterns 匹配字符串数组
 * order patterns中的字符串是否要按顺序搜索
 * percent 匹配成功百分比达到多少认定成功
 * @return
 */
bool fuzzyMatch(const std::string &data,
                const std::vector<std::string> &patterns, bool order,
                int percent) {
  if (data.size() == 0 || patterns.size() == 0 || percent > 100 ||
      percent < 0) {
    return false;
  }
  ...
  int per = std::round(static_cast<float>(match_cnt) / patterns.size() * 100);
  // std::cout << "match_cnt: " << match_cnt << " match percent: " << per << std::endl;
  if (per >= percent) {
    return true;
  } else {
    return false;
...
HttpHealthCheckerImpl::HttpActiveHealthCheckSession::healthCheckResult() {
  ...
  if (!parent_.http_status_checker_.inExpectedRanges(response_code)) {
    ...
    ENVOY_CONN_LOG(debug, "hc dip={}, response={} response data={}", *client_, host_->healthCheckAddress()->asString(), response_code, response_data_);
    host_->healthFailReasonSet(Host::HealthFailReason::CODE_UNMATCH);

    std::transform(response_data_.begin(), response_data_.end(), response_data_.begin(), ::tolower);
    switch(response_code) {
      case 302:
        if (parent_.host_value_.empty() ||
            (parent_.cluster_.info() && (parent_.host_value_ == parent_.cluster_.info()->name()))) {
          ENVOY_CONN_LOG(debug, "hc dip={}, host_value={} cluster name={}", *client_,
                         host_->healthCheckAddress()->asString(), parent_.host_value_,
                         parent_.cluster_.info()->name());
          auto location = response_headers_->getLocationValue();
          if (!location.empty()) {
            host_->healthFailReasonSet(Host::HealthFailReason::HOST_ERROR);
          }
        }
      break;
      case 400:
        if (fuzzyMatch(response_data_, {"http", "request", "to", "https"}, true, 80)) {
          // Client sent an HTTP request to an HTTPS server
          // the plain http request was sent to https port
          host_->healthFailReasonSet(Host::HealthFailReason::NO_TLS_AUTH);
        } else {
          host_->healthFailReasonSet(Host::HealthFailReason::PATH_ERROR);
        }
      break;
      case 403:
        if (std::string::npos != response_data_.find("403 forbidden")) {
          host_->healthFailReasonSet(Host::HealthFailReason::HOST_ERROR);
        } else {
          host_->healthFailReasonSet(Host::HealthFailReason::SERVER_NAME_ERROR);
        }
      break;
      case 404:
        if (fuzzyMatch(response_data_, {"404", "page", "not", "found"}, false, 80)) {
          //404 page not found
          host_->healthFailReasonSet(Host::HealthFailReason::PATH_ERROR);
        } else {
          host_->healthFailReasonSet(Host::HealthFailReason::HOST_ERROR);
...
void TcpHealthCheckerImpl::TcpActiveHealthCheckSession::onData(Buffer::Instance& data) {
  ...
  if (TcpHealthCheckMatcher::match(parent_.receive_bytes_, data)) {
    ...
    ENVOY_CONN_LOG(trace, "dip={}", *client_, host_->healthCheckAddress()->asString());
    host_->healthFailReasonSet(Host::HealthFailReason::TCP_RECV_CONTENT_UNMATCH);
  }
}
...
PingHealthCheckerImpl::PingHealthCheckerImpl(const Cluster& cluster,
                                           const envoy::config::core::v3::HealthCheck& config,
                                           Event::Dispatcher& dispatcher, Runtime::Loader& runtime,
                                           Random::RandomGenerator& random,
                                           HealthCheckEventLoggerPtr&& event_logger)
...
PingHealthCheckerImpl::PingActiveHealthCheckSession::~PingActiveHealthCheckSession() {
  onDeferredDelete();
}

void PingHealthCheckerImpl::PingActiveHealthCheckSession::onDeferredDelete() {
  if (icmp_client_) {
    icmp_close(icmp_client_);
    icmp_client_ = nullptr;
  }
}

bool PingHealthCheckerImpl::PingActiveHealthCheckSession::doPing() {
  const char* host = host_->healthCheckAddress()->ip()->addressAsString().c_str();
  ENVOY_LOG(debug, "hc dip={}, start ping host={}", host_->healthCheckAddress()->asString(), host);

  if (!icmp_client_) {
    if (!(icmp_client_ = icmp_open(host, 0x1337, 0))) {
      ENVOY_LOG(error, "hc dip={}, icmp open failed", host_->healthCheckAddress()->asString());
      return false;
    }
    ENVOY_LOG(debug, "hc dip={}, icmp open success", host_->healthCheckAddress()->asString());
  }

  if (icmp_send(icmp_client_, ICMP_ECHO, 0, 0)) {
    ENVOY_LOG(error, "hc dip={}, icmp echo failed", host_->healthCheckAddress()->asString());
    return false;
  }
  ENVOY_LOG(debug, "hc dip={}, icmp echo success", host_->healthCheckAddress()->asString());

  return true;
}

void PingHealthCheckerImpl::PingActiveHealthCheckSession::onInterval() {
  if (doPing()) {
    polling_timer_->enableTimer(icmp_polling_interval_);
  } else {
    handleFailure(envoy::data::core::v3::NETWORK);
  }
}

void PingHealthCheckerImpl::PingActiveHealthCheckSession::onPollingTimer() {
  if (!icmp_client_) {
    return;
  }

  char buf[BUFSIZ];
  int ret = icmp_recv_noblock(icmp_client_, ICMP_ECHOREPLY, buf, sizeof(buf));
  ENVOY_LOG(debug, "hc dip={} recv ret: {}", host_->healthCheckAddress()->asString(), ret);
  if (ret >= 0) {
    handleSuccess(false);
  } else if (ret == -2 ) {
    polling_timer_->enableTimer(icmp_polling_interval_);
...
void PingHealthCheckerImpl::PingActiveHealthCheckSession::onTimeout() {
  if (polling_timer_ != nullptr) {
    polling_timer_->disableTimer();
  }
}
