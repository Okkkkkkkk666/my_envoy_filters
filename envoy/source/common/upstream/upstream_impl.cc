...
HostDescriptionImpl::HostDescriptionImpl(
    ...
    uint32_t priority, TimeSource& time_source, const std::string& dns_addr)
    : cluster_(cluster), hostname_(hostname),
      ...
      creation_time_(time_source.monotonicTime()), dns_addr_(dns_addr) {
...
const std::unordered_map<Host::HealthFailReason, int> HostImpl::health_fail_reason_prior_ = {
    {Host::HealthFailReason::NONE, INT_MAX},
    {Host::HealthFailReason::NO_TLS_AUTH, 1},
    {Host::HealthFailReason::SERVER_NAME_ERROR, 1},
    {Host::HealthFailReason::HOST_ERROR, 2},
    {Host::HealthFailReason::PATH_ERROR, 2},
    {Host::HealthFailReason::CODE_UNMATCH, 3},
    {Host::HealthFailReason::TCP_RECV_CONTENT_UNMATCH, 3},
    {Host::HealthFailReason::CONNECT_FAILED, 4}};
...
void PriorityStateManager::registerHostForPriority(
  ...
  const auto host = std::make_shared<HostImpl>(
      ...
      locality_lb_endpoint.priority(), lb_endpoint.health_status(), time_source, dns_addr);
