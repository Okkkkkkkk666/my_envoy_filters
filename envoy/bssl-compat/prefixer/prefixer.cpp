...
void MyFrontendAction::EndSourceFileAction() {
...

  {
    ...
    cstr << "//" << std::endl << "// THIS FILE IS GENERATED BY THE PREFIXER TOOL DO NOT EDIT" << std::endl << "//" << std::endl
         ...
         << std::endl
         << "#define LIBCRYPTO_SO \"/opt/envoy/lib/libcrypto.so.3" << (m_shlibversion.size() ? "." + m_shlibversion : "") << "\"" << std::endl
         << "#define LIBSSL_SO \"/opt/envoy/lib/libssl.so.3" << (m_shlibversion.size() ? "." + m_shlibversion : "") << "\"" << std::endl
         << std::endl
         ...
         << "  const char* crypto_path = LIBCRYPTO_SO;" << std::endl
         << "  if (access(LIBCRYPTO_SO, F_OK) != 0) {" << std::endl
         << "    crypto_path = \"libcrypto.so.3\";  // 回退到系统库" << std::endl
         << "  }" << std::endl
         << std::endl
         << "  if((libcrypto = dlopen(crypto_path, RTLD_NOW | RTLD_LOCAL)) == NULL) {" << std::endl
         << "    fprintf(stderr, \"dlopen(%s) : %s\\n\", crypto_path, dlerror());" << std::endl
         << "    exit(ELIBACC);" << std::endl
         << "  }" << std::endl
         << std::endl
         << "  const char* ssl_path = LIBSSL_SO;" << std::endl
         << "  if (access(LIBSSL_SO, F_OK) != 0) {" << std::endl
         << "    ssl_path = \"libssl.so.3\";  // 回退到系统库" << std::endl
         << "  }" << std::endl
         << std::endl
         << "  if((libssl = dlopen(ssl_path, RTLD_NOW | RTLD_LOCAL)) == NULL) {" << std::endl
         << "    fprintf(stderr, \"dlopen(%s) : %s\\n\", ssl_path, dlerror());" << std::endl
         << "    exit(ELIBACC);" << std::endl
         << "  }" << std::endl
         << std::endl
