...
public:
  ...
  /**
   * @return span tags according to the tracing configuration.
   */
  virtual const std::set<std::string>& getSpanTagsFromTracing() const PURE;

  /**
   * @return check whether the tag is configured
   */
  virtual bool hasSpanTag(const std::string& tag) const PURE;
};
