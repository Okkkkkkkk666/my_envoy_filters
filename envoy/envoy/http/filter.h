...
public:
  ...
  /**
   * 返回默认的缓冲区大小。这个值是由config.listener.v3.Listener.per_connection_buffer_limit_bytes
   * 指定的。该值不受setDecoderBufferLimit的影响。
   * @return 默认的缓冲区大小，单位字节
   */
  virtual uint32_t defaultDecoderBufferLimit() PURE;
...
public:
  ...
  /**
   * 返回默认的缓冲区大小。这个值是由config.listener.v3.Listener.per_connection_buffer_limit_bytes
   * 指定的。该值不受setEncoderBufferLimit的影响。
   * @return 默认的缓冲区大小，单位字节
   */
  virtual uint32_t defaultEncoderBufferLimit() PURE;
