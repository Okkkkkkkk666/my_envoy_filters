...
#include "envoy/srhino_plugin_framework/v1_4_x/libs/net/net.h"
#include "envoy/srhino_plugin_framework/v1_4_x/test/libs/net/grpc_call.h"
#include "envoy/srhino_plugin_framework/v1_4_x/test/libs/net/http_call.h"
...
public:
  MockNet() {
    ...
    ON_CALL(*this, createGrpcCall(_, _, _, _)).WillByDefault(Return(grpc_call_ptr_));
  }

public:
  MOCK_METHOD(HttpCallSharedPtr, createHttpCall,
              (const std::string&, const std::chrono::milliseconds&, bool), ());
  MOCK_METHOD(GrpcCallSharedPtr, createGrpcCall,
              (const std::string&, const std::string&, const std::chrono::milliseconds&, bool), ());
