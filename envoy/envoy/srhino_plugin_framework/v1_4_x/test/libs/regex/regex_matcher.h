...
public:
  MockRegexMatcher() {
    EXPECT_CALL(*this,
                init(A<const std::vector<ExpressionView>&>(), A<std::vector<ExpressionView>&>()))
        .WillRepeatedly(Return(true));
    EXPECT_CALL(*this, init(A<const std::vector<RuleConfig>&>(), A<std::vector<RuleConfig>&>()))
        .WillRepeatedly(Return(true));
    ...
    EXPECT_CALL(*this, pcreMatch(_, _, _, _))
        .WillRepeatedly(
            testing::Invoke([&](const char* data, size_t len, std::vector<MatchResult>& results,
                                EncodingType encode) -> bool {
              if (!match_results_.empty()) {
    ...
    EXPECT_CALL(*this, pcreMatch(A<const std::string&>(), _, _))
        .WillRepeatedly(
            testing::Invoke([&](const std::string& data, std::vector<MatchResult>& results,
                                EncodingType encode) -> bool {
              if (!match_results_.empty()) {
    ...
    EXPECT_CALL(*this, pcreMatch(A<const std::string_view&>(), _, _))
        .WillRepeatedly(
            testing::Invoke([&](const std::string_view& data, std::vector<MatchResult>& results,
                                EncodingType encode) -> bool {
              if (!match_results_.empty()) {
    ...
    EXPECT_CALL(*this, pcreBlockMatch(_, _, _, _))
        .WillRepeatedly(
            testing::Invoke([&](const char* data, size_t len, std::vector<MatchResult>& results,
                                EncodingType encode) -> bool {
              if (!match_results_.empty()) {
    ...
    EXPECT_CALL(*this, pcreBlockMatch(A<const std::string&>(), _, _))
        .WillRepeatedly(
            testing::Invoke([&](const std::string& data, std::vector<MatchResult>& results,
                                EncodingType encode) -> bool {
              if (!match_results_.empty()) {
    ...
    EXPECT_CALL(*this, pcreBlockMatch(A<const std::string_view&>(), _, _))
        .WillRepeatedly(
            testing::Invoke([&](const std::string_view& data, std::vector<MatchResult>& results,
                                EncodingType encode) -> bool {
              if (!match_results_.empty()) {
    ...
    EXPECT_CALL(*this, dataTagPcreBlockMatch(A<const char*>(), _, _, _))
        .WillRepeatedly(
            testing::Invoke([&](const char* data, size_t len, std::vector<MatchResult>& results,
                                bool left_most) -> bool {
              if (!match_results_.empty()) {
    ...
    EXPECT_CALL(*this, dataTagPcreBlockMatch(A<const std::string&>(), _, _))
        .WillRepeatedly(
            testing::Invoke([&](const std::string& data, std::vector<MatchResult>& results,
                                bool left_most) -> bool {
              if (!match_results_.empty()) {
    ...
    EXPECT_CALL(*this, dataTagPcreBlockMatch(A<const std::string_view&>(), _, _))
        .WillRepeatedly(
            testing::Invoke([&](const std::string_view& data, std::vector<MatchResult>& results,
                                bool left_most) -> bool {
              if (!match_results_.empty()) {
...
public:
  ...
  MOCK_METHOD(bool, init, (const std::vector<RuleConfig>&, std::vector<RuleConfig>&), ());
  ...
  MOCK_METHOD(bool, pcreMatch, (const char*, size_t, std::vector<MatchResult>&, EncodingType), ());
  MOCK_METHOD(bool, pcreMatch, (const std::string&, std::vector<MatchResult>&, EncodingType), ());
  MOCK_METHOD(bool, pcreMatch, (const std::string_view&, std::vector<MatchResult>&, EncodingType),
              ());
  MOCK_METHOD(bool, pcreBlockMatch, (const char*, size_t, std::vector<MatchResult>&, EncodingType),
              ());
  MOCK_METHOD(bool, pcreBlockMatch, (const std::string&, std::vector<MatchResult>&, EncodingType),
              ());
  MOCK_METHOD(bool, pcreBlockMatch,
              (const std::string_view&, std::vector<MatchResult>&, EncodingType), ());
  MOCK_METHOD(bool, dataTagPcreBlockMatch, (const char*, size_t, std::vector<MatchResult>&, bool),
              ());
  MOCK_METHOD(bool, dataTagPcreBlockMatch, (const std::string&, std::vector<MatchResult>&, bool),
              ());
  MOCK_METHOD(bool, dataTagPcreBlockMatch,
              (const std::string_view&, std::vector<MatchResult>&, bool), ());
  ...
  MOCK_METHOD(bool, isEngineReady, (EncodingType), (const));
