...
enum class HealthFailReason {
  NONE = 0,
  CONNECT_FAILED = 1, // 连接失败，可能是网络不通
  CODE_UNMATCH = 2, // 响应码不匹配
  NO_TLS_AUTH = 3, // 主机是https，未配置tls认证
  SERVER_NAME_ERROR = 4, // 可能是sni未配置
  HOST_ERROR = 5, // host不匹配
  PATH_ERROR = 6, // 路径不存在
  TCP_RECV_CONTENT_UNMATCH = 7, // tcp接收内容不匹配
  // HTTP_PROTOCOL_ERROR = 7, // http类型不正确
};
  ...
  virtual void healthFailReasonSet(HealthFailReason reason) PURE;
  virtual Host::HealthFailReason healthFailReasonGet() const PURE;
  virtual void healthFailReasonClear() PURE;
  /**
...
public:
  ...
  /**
   * @param global 指定cluster销毁时指标是否被销毁
   ...
   */
  virtual Stats::Scope& statsScope(bool global = false) const PURE;
