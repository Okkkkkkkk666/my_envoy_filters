...
def _org_brotli():
    external_http_archive(
        name = "org_brotli",
        patch_args = ["-p1"],
        patches = ["@envoy//bazel:org_brotli.patch"],
    )
    native.bind(
        name = "brotlienc",
...
def _proxy_wasm_cpp_sdk():
    external_http_archive(
        ...
        patch_args = ["-p1"],
        patches = ["@envoy//bazel:proxy_wasm_cpp_sdk.patch"],
    )
...
def _com_github_libicmp():
    external_http_archive(
        name = "com_github_libicmp",
        build_file = "@envoy//bazel/external:libicmp.BUILD",
        patch_args = ["-p1"],
        patches = ["@envoy//bazel:libicmp.patch"],
    )
    native.bind(
        name = "libicmp",
        actual = "@com_github_libicmp//:libicmp",
    )

def _cn_ouryun_libnsq():
    external_http_archive(
        name = "cn_ouryun_libnsq",
        build_file = "@envoy//bazel/external:libnsq.BUILD",
    )
    native.bind(
        name = "libnsq",
        actual = "@cn_ouryun_libnsq//:libnsq",
    )