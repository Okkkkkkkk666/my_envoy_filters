...
void SmtpDecoder::onData(const Buffer::Instance& data) {
  ...
  if (session_.state == State::StreamingData) {
    ...
    if (pos >= 0) {
      callbacks_.onDataChunk(pos);
      decode_buffer_.drain(pos + 5);
      switchToCmdMode();
      callbacks_.onCommand("DATA_END", "");
      ...
      callbacks_.onDataChunk(decode_buffer_.length());
  ...
  while (decode_buffer_.length() > 0) {
    ...
    if (index == -1) {
      if (decode_buffer_.length() > max_line_length_) {
        callbacks_.onProtocolError("Line length exceeds the maximum allowed");
        decode_buffer_.drain(decode_buffer_.length());
        return;
      }
      return;
    ...
    if (is_starttls && decode_buffer_.length() > 0) {
      callbacks_.onProtocolError("554 5.5.1 Command pipelining not allowed after STARTTLS\r\n");
      decode_buffer_.drain(decode_buffer_.length());
      return; 
    }
...
bool SmtpDecoder::handleCommandWithState(absl::string_view command, absl::string_view args) {
  ...
  if (absl::EqualsIgnoreCase(command, "RSET")) {
    session_.reset();
    session_.state =
        session_.state == State::Init || session_.state == State::WaitHeloOrEhlo
            ? State::WaitHeloOrEhlo
            : State::WaitMailFrom;
  ...
  switch (session_.state) {
    ...
    case State::WaitDataCmd:
      ...
      if (absl::EqualsIgnoreCase(command, "DATA")) {
        if (session_.rcpt_to_list.empty()) {
          return false;
        }
