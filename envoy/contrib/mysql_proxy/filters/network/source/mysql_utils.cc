...
void BufferHelper::addLengthEncodedString(Buffer::Instance& buffer, const std::string& str) {
  addLengthEncodedInteger(buffer, str.size());
...
DecodeStatus BufferHelper::readLengthEncodedString(Buffer::Instance& buffer, std::string& str) {
  uint64_t len = 0;
  if (readLengthEncodedInteger(buffer, len) != DecodeStatus::Success) {
    return DecodeStatus::Failure;
  }
  return readStringBySize(buffer, len, str);
}
