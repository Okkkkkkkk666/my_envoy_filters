...
TEST_F(HttpConnectionManagerConfigTest, TracingSpanTagsConfig) {
  const std::string yaml_string = R"EOF(
...
tracing:
  span_tags: ["guid:x-request-id", "http.url", "http.method"]
  )EOF";
  HttpConnectionManagerConfig config(parseHttpConnectionManagerFromYaml(yaml_string), context_,
                                     date_provider_, route_config_provider_manager_,
                                     scoped_routes_config_provider_manager_, http_tracer_manager_,
  ...
  std::set<std::string> span_tags{"guid:x-request-id", "http.url", "http.method"};
  std::set<std::string> span_tags_ = config.tracingConfig()->span_tags_;
  EXPECT_EQ(span_tags, span_tags_);
}
