...
#include "source/common/srhino_plugin_framework/v1_0_x/libs/regex/regex_matcher_impl.h"
...
using namespace SrhinoPluginFramework::v1_0_x::Libs::Regex;
...
TEST(RegexMatcherTest, Serialize) {
  ...
  for (int i = 0; i < EncodingType::MAX; i++) {
    std::string del_path = db_path + "_encoding" + std::to_string(i) + ".sdb";
    EXPECT_EQ(std::remove(del_path.c_str()), 0);
  }
}
