...
TEST_F(ConnPoolImplDispatcherBaseTest, SingleHostReqCxActive) {
  newActiveClientAndStream();
  std::string host_cx_active = clients_.back()->parent_.host()->address()->asString() + ".upstream_cx_active";
  auto& host_cx_active_counter = clients_.back()->parent_.host()->cluster().statsScope().gaugeFromString(host_cx_active, Stats::Gauge::ImportMode::NeverImport);
  uint64_t host_upstream_cx_active = host_cx_active_counter.value();
  ASSERT_EQ(1, host_upstream_cx_active);

  closeStreamAndDrainClient();
  host_upstream_cx_active = host_cx_active_counter.value();
  std::cout<< "host_upstream_cx_active:"<<host_upstream_cx_active<<std::endl;
  ASSERT_EQ(0, host_upstream_cx_active);

}

TEST_F(ConnPoolImplDispatcherBaseTest, SingleHostReqTotal) {
  newActiveClientAndStream();
  std::string host_rq_total = host_->address()->asString() + ".upstream_rq_total";
  auto& host_rq_total_counter = host_->cluster().statsScope().gaugeFromString(host_rq_total, Stats::Gauge::ImportMode::NeverImport);
  uint64_t value = host_rq_total_counter.value();
  ASSERT_EQ(1, value);

  closeStreamAndDrainClient();
  value = host_rq_total_counter.value();
  ASSERT_EQ(1, value);
}
