...
TEST(ContextProviderTest, NodeSniContext) {
  envoy::config::core::v3::Node node;
  TestUtility::loadFromYaml(R"EOF(
  id: some_id
  cluster: some_cluster
  metadata:
    sni: helloworld
                            ...
                            node);
  auto it = node.metadata().fields().find("sni");
  EXPECT_NE(it, node.metadata().fields().end());
  EXPECT_EQ("helloworld", node.metadata().fields().at("sni").string_value());
  }
