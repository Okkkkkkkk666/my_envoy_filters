...
public:
  ...
  void expectSuccessStartFailedFailFirst(
    ...
    std::string cluster_host_health_status = cluster_->prioritySet().getMockHostSet(0)->hosts_[0]->address()->asString() + ".health_check.healthy";
    uint64_t value = cluster_->prioritySet().getMockHostSet(0)->hosts_[0]->cluster().statsScope().gaugeFromString(cluster_host_health_status, Stats::Gauge::ImportMode::NeverImport).value();
    EXPECT_EQ(static_cast<uint64_t>(Host::Health::Unhealthy), value);
...
TEST_F(TcpHealthCheckerImplTest, PassiveFailure) {
  ...
  std::string cluster_host_health_status = cluster_->prioritySet().getMockHostSet(0)->hosts_[0]->address()->asString() + ".health_check.healthy";
  uint64_t value = cluster_->prioritySet().getMockHostSet(0)->hosts_[0]->cluster().statsScope().gaugeFromString(cluster_host_health_status, Stats::Gauge::ImportMode::NeverImport).value();
  EXPECT_EQ(static_cast<uint64_t>(Host::Health::Healthy), value);
