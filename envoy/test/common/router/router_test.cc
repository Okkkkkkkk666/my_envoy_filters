...
TEST_F(RouterTest, ConfigNoHealthyinfo) {
  const std::string& str = "没有健康的上游业务";
  ConstStringOptRef details = str;
  EXPECT_CALL(runtime_.snapshot_, get("NoHealthyUpstream"))
      .Times(AtLeast(1))
      .WillRepeatedly(Return(details));
  EXPECT_TRUE(runtime_.snapshot_.get("NoHealthyUpstream").has_value());
  EXPECT_EQ("没有健康的上游业务", runtime_.snapshot_.get("NoHealthyUpstream").value().get());
  ...
  EXPECT_CALL(callbacks_, sendLocalReply(Http::Code::ServiceUnavailable, _, _, _, _))
      .WillOnce(Invoke([&](Http::Code code, absl::string_view body,
        ...
        EXPECT_EQ(details, "no_healthy_upstream");
      }));
...
TEST_F(RouterTest, RetryUpstream5xxNotComplete) {
  ...
  EXPECT_EQ(1U, cm_.thread_local_cluster_.cluster_.info_->stats_store_
                    ...
                    .value());
  std::string upstream_host_rq_2xx =
      cm_.thread_local_cluster_.conn_pool_.host_->address()->asString() + ".upstream_rq_2xx";
  uint64_t host_rq_2xx_value =
      cm_.thread_local_cluster_.conn_pool_.host_->cluster()
          .statsScope()
          .gaugeFromString(upstream_host_rq_2xx, Stats::Gauge::ImportMode::NeverImport)
          .value();
  EXPECT_EQ(1U, host_rq_2xx_value);
}
