...
TEST_F(RouterTestSupressGRPCStatsEnabled, ExcludeTimeoutHttpStats) {
  ...
  std::string upstream_host_rq_5xx = cm_.thread_local_cluster_.conn_pool_.host_->address()->asString() + ".upstream_rq_5xx";
  uint64_t host_rq_5xx_value = cm_.thread_local_cluster_.conn_pool_.host_->cluster().statsScope().gaugeFromString(upstream_host_rq_5xx, Stats::Gauge::ImportMode::NeverImport).value();
  EXPECT_EQ(0U, host_rq_5xx_value);
}
